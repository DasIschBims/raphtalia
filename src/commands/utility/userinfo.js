  ...
  async execute(interaction) {
    async function getUserBannerUrl(userId) {
      ...
      const bannerColor = user.banner_color
        ? `Banner Color ${user.banner_color}`
        : "User didn't set a banner color/banner";
      return user.banner
        ? `[Link (4096x4096)](https://cdn.discordapp.com/banners/${userId}/${user.banner}.png?size=4096)\n[Link (2048x2048)](https://cdn.discordapp.com/banners/${userId}/${user.banner}.png?size=2048)`
        : `${bannerColor}`;
    }
    ...
    async function getUserAvatarUrl(userId) {
      const user = await interaction.client.api.users(userId).get();
      return user.avatar
        ? `[Link (4096x4096)](https://cdn.discordapp.com/avatars/${userId}/${user.avatar}.png?size=4096)\n[Link (2048x2048)](https://cdn.discordapp.com/avatars/${userId}/${user.avatar}.png?size=2048)`
        : `${bannerColor}`;
    }
    ...
    async function sendUserEmbed() {
      if (interaction.options.getMember("user") === null) {
        ...
        let userbanner = await getUserBannerUrl(
          interaction.options.getMember("user").id
        );

        let useravatar = await getUserAvatarUrl(
          interaction.options.getMember("user").id
        );
