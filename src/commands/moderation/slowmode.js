...
module.exports = {
     data: new SlashCommandBuilder()
             ...
             .addNumberOption(set => set.setName("time").setDescription("Enter slowmode time (in seconds), max is 21600 seconds (6 hours). Enter 0 to remove slowmode.").setRequired(true))
             .addChannelOption(channel => channel.setName("channel").setDescription("Pick a channel to set slowmode for.").setRequired(true)),
  execute(interaction) {
      ...
      if (!interaction.member.permissions.has(Permissions.FLAGS.MANAGE_CHANNELS)) {
        ...
        if (!interaction.guild.members.cache.get(appId).permissions.has(Permissions.FLAGS.MANAGE_CHANNELS)) {
          {
               interaction.reply({ephemeral: true, embeds: [
                 new MessageEmbed()
          ...
          if (channeltype === 'GUILD_TEXT' || 'GUILD_THREAD') {
            if (time === 0) {
                ...
                interaction.reply({ephemeral: true, embeds: [
                    new MessageEmbed()
                    .setTitle(`Successfully removed slowmode for #${pickedchannel.name}! :arrows_clockwise:`)
                    .setColor("#58ff8d")
                    .setTimestamp()
                ]})
              } else {
                ...
                interaction.reply({ephemeral: true, embeds: [
                    new MessageEmbed()
                    .setTitle(`Successfully set slowmode to ${time} seconds for #${pickedchannel.name}! :stopwatch:`)
                    .setColor("#58ff8d")
                    .setTimestamp()
                  ]})
              }
